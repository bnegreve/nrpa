...
class Board {
 public:
  ...
  double score (int n = numberHats) {
    int nb = 0;
    if (n == 0) {
      bool found = true, atLeastOne = false;
      /*     for (int i = 0; i < numberHats; i++)  */
      /*       fprintf (stderr, "%d", b [i + 1]);  */
      /*     fprintf (stderr, "\n");  */
      for (int prisonner = 0; ((prisonner < numberHats) && found); prisonner++) {
	variableGP [0] = prisonner;
	int index = 3, nbZeros = 0, nbOnes = 0;
	for (int i = 0; i < numberHats; i++) 
	  if (i != prisonner) {
	    variableGP [index] = b [i + 1];
	    index++;
	    if (b [i + 1] == 0)
	      nbZeros++;
	    else
	...
	variableGP [1] = nbZeros;
	variableGP [2] = nbOnes;
	int depth = 0;
	float eval = evalStack (depth);
	if ((eval >= 1) && (b [prisonner + 1] == 1))
	  atLeastOne = true;
	if ((eval >= 1) && (b [prisonner + 1] == 0))
	  found = false;
	if ((eval <= 0) && (b [prisonner + 1] == 0))
	  atLeastOne = true;
	if ((eval <= 0) && (b [prisonner + 1] == 1))
	  found = false;
      }
      if (found && atLeastOne)
	return 1.0;
...
void initVariables () {
  tableFunctor.add ("me", 0, true);
  tableFunctor.add ("O's", 0, true);
  tableFunctor.add ("1's", 0, true);
  for (int i = 0; i < numberHats - 1; i++) {
...
void initFunctors () {
  ...
  tableFunctor.add ("exp", 1, true);
  tableFunctor.add ("pow", 2, true);
  tableFunctor.add ("and", 2, true);
  tableFunctor.add ("or", 2, true);
  tableFunctor.add ("xor", 2, true);
  tableFunctor.add ("not", 1, true);
  tableFunctor.add ("!=", 2, true);
  tableFunctor.add ("0", 0, true);
